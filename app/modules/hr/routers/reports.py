...
from app.modules.hr.schemas.reports import (ReportCommentIn, ReportEntryIn, ReportOut, ReportUpdate)
...
async def submit_reports(entries: list[ReportEntryIn], user: CurrentUser, db: DB):
    ...
    from app.modules.hr.models.misc import HealthCheckinPrompt
    ...
    try:
        from app.modules.hr.workers.tasks.ai_workload import analyze_report
        from app.modules.hr.workers.tasks.work_thread_matching import match_report
        for r in reports:
...
async def no_project_today(user: CurrentUser, db: DB):
    ...
    from sqlalchemy import select
    from app.modules.hr.models.misc import Alert, AlertSetting
...
async def no_project_notifications(db: DB, user=DashUser):
    ...
    from sqlalchemy import select
    from app.modules.hr.models.misc import Alert
...
async def get_report(report_id: int, db: DB, user: CurrentUser):
    ...
    from app.modules.hr.models.reports import Project, Report, ReportComment
    from app.core.models.user import User
